...
class KimiCLI:
    ...
    async def create(
        ...
        *,
        ...
        global_exclude_tools: list[str] | None = None,
        disable_curl_tip: bool = False,
    ) -> "KimiCLI":
        """
        ...
        runtime = await Runtime.create(config, llm, session, yolo, global_exclude_tools,disable_curl_tip)
    ...
    async def run_shell_mode(self, command: str | None = None,daemon: bool = False) -> bool:
        from kimi_cli.ui.shell import ShellApp, WelcomeInfoItem
        welcome_info = [
            WelcomeInfoItem(
                name="Directory", value=str(self._runtime.session.work_dir)
            ),
        ...
        with self._app_env():
            app = ShellApp(self._soul, welcome_info=welcome_info,daemon=daemon)

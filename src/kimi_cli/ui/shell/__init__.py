...
class ShellApp:
    ...
    async def run(self, command: str | None = None) -> bool:
        if self.daemon:
            ...
            if command is None or not command.strip():
                console.print(
                    "[red]Daemon mode requires --command to be provided[/red]"
                )
                return False
            logger.info(
                "Running in daemon mode with command: {command}", command=command
            )
                ...
                while True:
                    ...
                    try:
                        if (
                            ...
                            and self.soul.context.n_checkpoints > 0
                        ):
                        ...
                        logger.info("Daemon run cancelled by user")
                        console.print("[red]Interrupted by user[/red]")
                        break
                    except BaseException as e:
                        logger.exception("Daemon iteration failed with error:")
                        console.print(f"[red]Daemon iteration error: {e}[/red]")
                    await asyncio.sleep(10.0)
            except KeyboardInterrupt:
                logger.debug("Exiting daemon by KeyboardInterrupt")
                console.print("Bye!")
            return True
    ...
    async def _run_soul_command(
        ...
        try:
            ...
            if isinstance(e, APIStatusError) and e.status_code == 401:
                console.print(
                    "[red]Authorization failed, please check your API key[/red]"
                )
    ...
    async def _auto_update(self) -> None:
        ...
        if result == UpdateResult.UPDATE_AVAILABLE:
            ...
            toast(
                "auto updated, restart to use the new version",
                topic="update",
                duration=5.0,
            )
